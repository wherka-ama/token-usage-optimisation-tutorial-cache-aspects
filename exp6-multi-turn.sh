...
copilot -p "Explain the CAP theorem." --model "$COPILOT_MODEL" \
...
copilot -p "Now explain how it relates to eventual consistency." --model "$COPILOT_MODEL" \
  --output-format json --silent \
  --no-custom-instructions --disable-builtin-mcps --yolo \
  --session-id "$SESSION_ID" 2>/dev/null

# Turn 3 — continues appending; cached prefix grows
copilot -p "Give me a real-world example of each combination." --model "$COPILOT_MODEL" \
  --output-format json --silent \
  --no-custom-instructions --disable-builtin-mcps --yolo \
  --session-id "$SESSION_ID" 2>/dev/null
